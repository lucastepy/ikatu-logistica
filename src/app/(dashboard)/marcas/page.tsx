...
export default function MarcasPage() {
  const { isHidden, isReadOnly, loadingRestrictions } = useFieldSecurity("Marca");
  ...
  const openEdit = (item: any) => {
    setEditingItem(item);
    setFormData({ nombre: item.marca_nombre });
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingItem ? `/api/marcas/${editingItem.marca_id}` : "/api/marcas";
      ...
      if (res.ok) {
        ...
        showToast(editingItem ? "Marca actualizada" : "Marca creada");
        fetchData();
      } else {
  ...
  const onConfirmDelete = async () => {
    ...
    const res = await fetch(`/api/marcas/${itemToDelete.marca_id}`, { 
      method: "DELETE",
      headers: {
        "x-tenant-id": tenantId,
        "x-user-email": user?.email || "",
  ...
  const filteredData = data.filter(item => 
    ...
    item.marca_id.toString().includes(searchTerm)
  );
  ...
  return (
    <div className="p-8 space-y-6 relative animate-in fade-in duration-500">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-accent">Marcas</h1>
          <p className="text-muted mt-1 font-medium italic">Gestión de marcas de activos y mercaderías.</p>
        </div>
        <Button onClick={openCreate} className="bg-accent text-white font-bold hover:brightness-105 h-11 px-6 rounded-xl shadow-lg shadow-accent/20 transition-all">
          <Plus className="h-4 w-4 mr-2 stroke-[3]" /> Nueva Marca
        </Button>
      </div>
      ...
      <Card className="bg-card border-none shadow-2xl rounded-3xl overflow-hidden">
        <CardHeader className="bg-slate-50/50 border-b border-slate-100 p-6">
           <div className="flex justify-between items-center">
              <div>
                <CardTitle className="text-lg font-bold text-slate-700">Catálogo de Marcas</CardTitle>
                <CardDescription className="text-xs">Marcas registradas para la operación logística.</CardDescription>
              </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-50/80 border-b border-slate-100 text-[11px] tracking-tight text-slate-400 font-bold uppercase">
                  {!isHidden("marca_id") && <th className="px-8 py-4 w-32 text-center">ID</th>}
                  {!isHidden("marca_nombre") && <th className="px-8 py-4">Nombre de la Marca</th>}
                  <th className="px-8 py-4 text-right">Acciones</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {loading ? (
                  <tr><td colSpan={3} className="px-8 py-10 text-center text-slate-400 italic">Cargando marcas...</td></tr>
                ) : currentItems.length === 0 ? (
                  <tr><td colSpan={3} className="px-8 py-10 text-center text-slate-400 italic">No se encontraron marcas registradas.</td></tr>
                ) : currentItems.map((item) => (
                  <tr key={item.marca_id} className="hover:bg-slate-50/30 transition-colors group">
                    {!isHidden("marca_id") && (
                      <td className="px-8 py-4 font-mono text-[11px] text-accent font-black text-center">
                         <span className="bg-accent/5 px-3 py-1 rounded-lg border border-accent/10">{item.marca_id}</span>
                      </td>
                    )}
                    {!isHidden("marca_nombre") && (
                      <td className="px-8 py-4">
                        <div className="flex items-center gap-3">
                          <div className="p-2 rounded-xl bg-slate-100 text-slate-400 group-hover:text-accent transition-colors">
                            <Tag className="h-4 w-4" />
                          </div>
                          <p className="font-bold text-slate-700 text-sm tracking-tight">{item.marca_nombre}</p>
                        </div>
                      </td>
                    )}
      ...
      <CustomModal 
        ...
        title={`${editingItem ? 'Editar' : 'Nueva'} Marca`}
        className="max-w-md shadow-[0_50px_100px_-20px_rgba(0,0,0,0.3)] border-white/50 backdrop-blur-xl"
      >
        <form onSubmit={handleSubmit} className="space-y-4 pt-2">
          {!isHidden("marca_id") && editingItem && (
            <div className="space-y-2">
              <Label className="flex items-center gap-2 text-slate-700 font-bold opacity-50">ID Interno <Hash className="h-3 w-3 text-accent" /></Label>
              <Input 
                value={editingItem.marca_id} 
                disabled
                className="h-11 font-mono font-bold bg-slate-50 border-slate-200 text-slate-400"
              />
            </div>
          )}
          ...
          {!isHidden("marca_nombre") && (
            <div className="space-y-2">
              <Label className="flex items-center gap-2 text-slate-700 font-bold">Nombre de la Marca <Tag className="h-3 w-3 text-accent" /></Label>
              <Input 
                value={formData.nombre} 
                onChange={e => setFormData({ nombre: e.target.value })} 
                placeholder="Ej: TOYOTA, SAMSUNG, etc." 
                required 
                autoFocus
                className="h-11 bg-white border-slate-200 text-slate-950 font-medium shadow-sm"
                disabled={isReadOnly("marca_nombre")}
              />
            </div>
          )}
      ...
      <ConfirmModal 
        ...
        description="Esta acción eliminará la marca permanentemente. Asegúrese de que no esté siendo utilizada por otros registros." 
        variant="light"
      />
    </div>
  );
