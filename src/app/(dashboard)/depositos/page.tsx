...
import { 
  Plus, Box, Edit3, Trash2, CheckCircle2, Save, 
  ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, Search,
  MapPin, Warehouse, ArrowRight, Map as MapIcon, Globe
} from "lucide-react";
...
interface Deposito {
  ...
  tipo?: { tipo_dep_dsc: string };
  departamento?: { dep_dsc: string };
  distrito?: { dis_dsc: string };
  ciudad?: { ciu_dsc: string };
  barrio?: { bar_dsc: string };
...
export default function DepositosPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const rTipos = await fetch("/api/admin/tipo-depositos");
      setTipos(await rTipos.json());
      
      const rDeps = await fetch("/api/admin/config-locations?type=dep");
  ...
  const onConfirmDelete = async () => {
    ...
    const res = await fetch(`/api/admin/depositos?id=${itemToDelete}`, { method: "DELETE" });
    if (res.ok) {
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-100 text-[11px] uppercase tracking-widest text-slate-400 font-black">
                  <th className="px-8 py-4 w-20 text-center">ID</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {loading ? (
                  ...
                  <tr key={item.deposito_id} className="hover:bg-slate-50/30 transition-colors group">
                    <td className="px-8 py-4 font-mono text-[11px] text-accent font-bold text-center">#{item.deposito_id}</td>
                    <td className="px-8 py-4">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                           <span className="font-bold text-slate-700 text-sm leading-none">{item.deposito_nombre}</span>
                        ...
                        <p className="text-[11px] text-slate-400 line-clamp-1">{item.deposito_direccion}</p>
                      </div>
                    </td>
                    <td className="px-8 py-4">
                      <div className="space-y-1">
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-6 pt-2 h-[80vh] overflow-hidden flex flex-col">
          <div className="flex-1 overflow-y-auto pr-2 space-y-6">
            <div className="grid grid-cols-2 gap-4">
               <div className="space-y-1.5">
                  ...
                  <Input value={formData.nombre} onChange={e => setFormData({...formData, nombre: e.target.value})} className="h-10 rounded-lg border-slate-200" required />
               </div>
               <div className="space-y-1.5">
                  <Label className="text-[10px] font-bold uppercase text-slate-400 tracking-wider">Tipo de Depósito</Label>
                  <select className="h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm outline-none" value={formData.tipoId} onChange={e => setFormData({...formData, tipoId: e.target.value})} required>
            ...
            <div className="grid grid-cols-2 gap-4">
               <div className="space-y-1.5">
                  ...
                  <select className="h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm" value={formData.depa} onChange={e => setFormData({...formData, depa: e.target.value, dis:"", ciu:"", bar:""})} required>
                     <option value="">Seleccione...</option>
               ...
               <div className="space-y-1.5">
                  ...
                  <select className="h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm" value={formData.dis} onChange={e => setFormData({...formData, dis: e.target.value, ciu:"", bar:""})} disabled={!formData.depa} required>
                     <option value="">Seleccione...</option>
            ...
            <div className="grid grid-cols-2 gap-4">
               <div className="space-y-1.5">
                  ...
                  <select className="h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm" value={formData.ciu} onChange={e => setFormData({...formData, ciu: e.target.value, bar:""})} disabled={!formData.dis} required>
                     <option value="">Seleccione...</option>
               ...
               <div className="space-y-1.5">
                  ...
                  <select className="h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm" value={formData.bar} onChange={e => setFormData({...formData, bar: e.target.value})} disabled={!formData.ciu}>
                     <option value="">Seleccione...</option>
            ...
            <div className="space-y-1.5">
               ...
               <Input value={formData.direccion} onChange={e => setFormData({...formData, direccion: e.target.value})} className="h-10 rounded-lg border-slate-200" placeholder="Ej: Avda. Principal c/ 10 de Agosto" />
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
               ...
               <div className="flex flex-col justify-end">
                  <div className="h-10 flex items-center justify-between px-4 bg-slate-50 border border-slate-100 rounded-lg">
                     ...
                     <input type="checkbox" checked={formData.estado} onChange={e => setFormData({...formData, estado: e.target.checked})} className="h-4 w-4 accent-accent" />
                  </div>
               </div>
            </div>
          ...
          <div className="flex gap-3 pt-6 border-t border-slate-100 mt-auto">
            <Button type="submit" className="flex-1 bg-accent text-white font-bold h-12 rounded-xl shadow-lg shadow-accent/20 gap-2"><Save className="h-4 w-4" /> {editingItem ? "Actualizar Almacén" : "Guardar Registro"}</Button>
