...
export async function GET() {
  try {
    const clientes = await prisma.clienteSaas.findMany({
      include: { plan: true },
      orderBy: { cli_saas_cod: 'asc' },
    });
    return NextResponse.json(clientes);
  } catch (error) {
    console.error("Error fetching clientes SaaS:", error);
    return NextResponse.json({ error: "Error al obtener clientes SaaS" }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    const lastCliente = await prisma.clienteSaas.findFirst({
      orderBy: { cli_saas_cod: 'desc' },
      select: { cli_saas_cod: true }
    });
    const nextCod = (lastCliente?.cli_saas_cod || 0) + 1;
    ...
    if (tenant) {
      ...
      if (existingTenant) {
        return NextResponse.json({ error: `El Tenant ID "${tenant}" ya está en uso por otro cliente.` }, { status: 400 });
      }
    }

    const cliente = await prisma.clienteSaas.create({
      data: {
    ...
    if (initInfra && tenant && sourceSchema) {
      try {
        ...
        for (const row of tables) {
          ...
          // 1. Crear la estructura en todos los casos
          await prisma.$executeRawUnsafe(`
            CREATE TABLE "${tenant}"."${tableName}" 
            (LIKE "${sourceSchema}"."${tableName}" INCLUDING ALL)
          `);
          
          // 2. Copiar datos SOLO si es una tabla de catálogo especificada
          if (tablesToPopulate.includes(tableName)) {
            await prisma.$executeRawUnsafe(`
              INSERT INTO "${tenant}"."${tableName}" 
              SELECT * FROM "${sourceSchema}"."${tableName}"
            `);
            console.log(`Datos migrados para tabla de catálogo: ${tableName}`);
          }
        }
        ...
        // Poblar la tabla empresa del nuevo tenant con sus datos iniciales
        await prisma.$executeRawUnsafe(`
          INSERT INTO "${tenant}"."empresa" 
          (empresa_nom, empresa_ruc, empresa_mail, empresa_dir, empresa_tel, empresa_propietario, empresa_nom_fan, empresa_estado, empresa_fecha_alta)
          VALUES 
          ($1, $2, $3, $4, $5, $6, $7, 'A', NOW())
        `, nombre, ruc, mail, direccion, telefono, propietario, nombre_fan);
    ...
    console.error("Error creating cliente SaaS:", error);
    return NextResponse.json({ error: "Error al crear cliente SaaS" }, { status: 500 });
  }
}
