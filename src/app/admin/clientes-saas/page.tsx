...
export default function ClientesSaasPage() {
  ...
  const [editingItem, setEditingItem] = useState<ClienteSaas | null>(null);
  ...
  const openEdit = (item: ClienteSaas) => {
    setEditingItem(item);
    setFormData({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingItem ? `/api/admin/clientes-saas/${editingItem.cli_saas_cod}` : "/api/admin/clientes-saas";
      ...
      if (res.ok) {
        ...
        showToast(editingItem ? "Cliente actualizado" : "Cliente creado", 'success');
        fetchData();
      } else {
  ...
  const onConfirmDelete = async () => {
    ...
    const res = await fetch(`/api/admin/clientes-saas/${itemToDelete}`, { method: "DELETE" });
    if (res.ok) {
  ...
  return (
    <div className="p-8 space-y-8 animate-in fade-in duration-500 relative min-h-screen">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tighter text-white">Clientes <span className="text-red-500">SaaS</span></h1>
          <p className="text-slate-400 mt-1 font-medium">Gestión de infraestructura y provisión de inquilinos (Tenants).</p>
        </div>
      ...
      <Card className="bg-slate-900/40 backdrop-blur-xl border-slate-800 shadow-2xl overflow-hidden">
        <CardHeader className="border-b border-slate-800 bg-slate-950/20 space-y-4 p-6">
          <div className="flex justify-between items-center">
            <div>
              <CardTitle className="text-lg font-bold text-white">Clientes Registrados</CardTitle>
              <CardDescription className="text-slate-500">Visualiza y administra los inquilinos del ecosistema SaaS.</CardDescription>
            </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-slate-800">
                {loading ? (
                  <tr><td colSpan={6} className="px-6 py-12 text-center text-slate-500 italic font-medium">Cargando inquilinos SaaS...</td></tr>
                ) : currentClientes.length === 0 ? (
                  <tr><td colSpan={6} className="px-6 py-12 text-center text-slate-500 italic font-medium">No se encontraron clientes.</td></tr>
          ...
          {!loading && filteredClientes.length > 0 && (
            <div className="p-6 border-t border-slate-800 flex items-center justify-between bg-slate-950/20">
              <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">
                Mostrando <span className="text-white font-black">{startIndex + 1}</span> a <span className="text-white font-black">{Math.min(startIndex + itemsPerPage, filteredClientes.length)}</span> de <span className="text-white font-black">{filteredClientes.length}</span> registros
              </p>
              <div className="flex items-center gap-2">
      ...
      <CustomModal 
        ...
        title={editingItem ? "Editar Cliente SaaS" : "Alta de Nuevo Cliente SaaS"}
        description="Configure la identidad y provisión del inquilino en la tabla clientes_saas."
        icon={Building2}
        className="max-w-4xl"
        variant="dark"
      >
        <form onSubmit={handleSubmit} className="space-y-6 pt-2">
          <div className="grid grid-cols-3 gap-4">
            <div className="space-y-2 col-span-2">
              <Label className="text-white font-bold text-xs tracking-widest">Nombre del Cliente / Razón Social</Label>
             ...
             <div className="space-y-4">
                <div className="space-y-2">
                  <Label className="text-white font-bold text-xs tracking-widest flex items-center gap-2">
                    ...
                    <div className="text-slate-500 cursor-help" title="Es el correo del administrador del sistema">
          ...
          <div className="flex gap-3 pt-6">
            <Button type="submit" disabled={isSubmitting} className="flex-1 bg-red-600 hover:bg-red-500 text-white font-bold gap-2 rounded-xl transition-all active:scale-95 py-6 uppercase tracking-widest text-xs disabled:opacity-70 disabled:scale-100">
              ...
              {isSubmitting ? "Guardando..." : (editingItem ? "Actualizar" : "Guardar Cliente SaaS")}
            </Button>
            <Button 
              variant="outline" 
      ...
      <ConfirmModal isOpen={isConfirmOpen} onClose={() => setIsConfirmOpen(false)} onConfirm={onConfirmDelete} title="¿Eliminar Cliente SaaS?" description="Esta acción es permanente. Solo se podrá eliminar si no existen usuarios vinculados a este inquilino." variant="dark" />
    </div>
  );
}
